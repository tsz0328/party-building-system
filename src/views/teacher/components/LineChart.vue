...
const initChart = () => {
  ...
  const series = props.series.map((item, index) => ({
    ...
    type: 'line' as const,
    data: item.values,
    lineStyle: { width: 2, color: props.colorPool[index % props.colorPool.length] },
    symbol: 'circle',
    symbolSize: 8,
    itemStyle: { color: props.colorPool[index % props.colorPool.length] },
    areaStyle: {
      color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
        { offset: 0, color: `${props.colorPool[index % props.colorPool.length]}33` },
        { offset: 1, color: `${props.colorPool[index % props.colorPool.length]}0a` },
      ]),
    },
  }))
